/*
 Adapted from the Adafruit and Xark's PDQ graphicstest sketch.

 See end of file for original header text and MIT license info.

 This sketch uses the GLCD font only.

 */

#ifdef min
#undef min
#endif
#ifdef max
#undef max
#endif
#include <algorithm>

#include <M5Atom.h>
#define LGFX_USE_V1
#include <LovyanGFX.hpp>
#include <lgfx_user/LGFX_ESP32_CVBS.hpp>

#include <esp32-hal-log.h>

static LGFX_CVBS   _cvbs;

unsigned long total = 0;
unsigned long tn = 0;

void printnice(int32_t v)
{
	char	str[32] = { 0 };
	sprintf(str, "%d", v);
	for (char *p = (str+strlen(str))-3; p > str; p -= 3)
	{
		memmove(p+1, p, strlen(p)+1);
		*p = ',';

	}
	while (strlen(str) < 10)
	{
		memmove(str+1, str, strlen(str)+1);
		*str = ' ';
	}
	_cvbs.println(str);
}

static inline uint32_t micros_start() __attribute__ ((always_inline));
static inline uint32_t micros_start()
{
	uint8_t oms = millis();
	while ((uint8_t)millis() == oms)
		;
	return micros();
}

uint32_t testHaD()
{
	// pseudo-code for cheesy RLE
	// start with color1
	// while more input data remaining
	// 	count =  0nnnnnnn = 1 byte or 1nnnnnnn nnnnnnnn 2 bytes (0 - 32767)
	// 	repeat color count times
	// 	toggle color1/color2
	static const uint8_t HaD_240x320[] PROGMEM =
	{
		,
	};

	_cvbs.fillScreen(TFT_BLACK);
  _cvbs.waitForFrame();

  uint32_t start = micros_start();

	for (int i = 0; i < 0x10; i++)
	{
		_cvbs.setAddrWindow(0, 0, 255, 239);

		uint16_t cnt = 0;
		uint16_t color = _cvbs.color332((i << 4) | i, (i << 4) | i, (i << 4) | i);
		uint16_t curcolor = 0;

		const uint8_t *cmp = &HaD_240x320[0];

    _cvbs.startWrite();
		while (cmp < &HaD_240x320[sizeof(HaD_240x320)])
		{
			cnt = pgm_read_byte(cmp++);
			if (cnt & 0x80) cnt = ((cnt & 0x7f) << 8) | pgm_read_byte(cmp++);
			_cvbs.pushBlock(curcolor, cnt);	// PDQ_GFX has count
			curcolor ^= color;
		}
		_cvbs.endWrite();
	}

	uint32_t t = micros() - start;

	_cvbs.setTextColor(TFT_YELLOW);
	_cvbs.setTextSize(2);
	_cvbs.setCursor(8, 285);
	_cvbs.print(F("http://hackaday.io/"));
	_cvbs.setCursor(96, 302);
	_cvbs.print(F("Xark"));

  _cvbs.waitForFrame();

	delay(3 * 1000L);

	return t;
}

uint32_t testFillScreen()
{
	uint32_t start = micros_start();
    // Shortened this tedious test!
		_cvbs.fillScreen(TFT_WHITE);
		_cvbs.fillScreen(TFT_RED);
		_cvbs.fillScreen(TFT_GREEN);
		_cvbs.fillScreen(TFT_BLUE);
		_cvbs.fillScreen(TFT_BLACK);

	return (micros() - start)/5;
}

uint32_t testText()
{
	_cvbs.fillScreen(TFT_BLACK);
	uint32_t start = micros_start();
	_cvbs.setCursor(0, 16);
	_cvbs.setTextColor(_cvbs.color16to8(TFT_WHITE), _cvbs.color16to8(TFT_BLACK));	_cvbs.setTextSize(1);
	_cvbs.println(F("Hello World!"));
  _cvbs.waitForFrame();
	_cvbs.setCursor(0, 16);
  _cvbs.setTextSize(2);
	_cvbs.setTextColor(_cvbs.color332(0xff, 0x00, 0x00));
	_cvbs.print(F("RED "));
  _cvbs.waitForFrame();
 return 0;
 	_cvbs.setTextColor(_cvbs.color332(0x00, 0xff, 0x00));
	_cvbs.print(F("GREEN "));
	_cvbs.setTextColor(_cvbs.color332(0x00, 0x00, 0xff));
	_cvbs.println(F("BLUE"));
  _cvbs.waitForFrame();


  _cvbs.setTextColor(TFT_YELLOW);
  _cvbs.setTextSize(2);
  _cvbs.println(1234.56);
  _cvbs.setTextColor(TFT_RED);
  _cvbs.setTextSize(3);
  _cvbs.println(0xDEADBEEF, HEX);
  _cvbs.println();
  _cvbs.setTextColor(TFT_GREEN);
  _cvbs.setTextSize(5);
  _cvbs.println(F("Groop"));
  _cvbs.setTextSize(2);
  _cvbs.println(F("I implore thee,"));
  _cvbs.setTextColor(TFT_GREEN);
	_cvbs.setTextSize(1);
	_cvbs.println(F("my foonting turlingdromes."));
	_cvbs.println(F("And hooptiously drangle me"));
	_cvbs.println(F("with crinkly bindlewurdles,"));
	_cvbs.println(F("Or I will rend thee"));
	_cvbs.println(F("in the gobberwarts"));
	_cvbs.println(F("with my blurglecruncheon,"));
	_cvbs.println(F("see if I don't!"));
	_cvbs.println(F(""));
	_cvbs.println(F(""));
	_cvbs.setTextColor(TFT_MAGENTA);
	_cvbs.setTextSize(6);
	_cvbs.println(F("Woot!"));
	uint32_t t = micros() - start;
	delay(1000);
	return t;
}

uint32_t testPixels()
{
	int32_t	w = _cvbs.width();
	int32_t	h = _cvbs.height();

	uint32_t start = micros_start();
	_cvbs.startWrite();
	for (uint16_t y = 0; y < h; y++)
	{
		for (uint16_t x = 0; x < w; x++)
		{
			_cvbs.drawPixel(x, y, _cvbs.color332(x<<3, y<<3, x*y));
		}
	}
	_cvbs.endWrite();
	return micros() - start;
}

uint32_t testLines(uint8_t color)
{
	uint32_t start, t;
	int32_t	x1, y1, x2, y2;
	int32_t	w = _cvbs.width();
	int32_t	h = _cvbs.height();

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	x1 = y1 = 0;
	y2 = h - 1;

	start = micros_start();

	for (x2 = 0; x2 < w; x2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}

  x2 = w - 1;

	for (y2 = 0; y2 < h; y2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}

	t = micros() - start; // fillScreen doesn't count against timing

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	x1 = w - 1;
	y1 = 0;
	y2 = h - 1;

	start = micros_start();

	for (x2 = 0; x2 < w; x2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}

	x2 = 0;
	for (y2 = 0; y2 < h; y2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}

	t += micros() - start;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	x1 = 0;
	y1 = h - 1;
	y2 = 0;

	start = micros_start();

	for (x2 = 0; x2 < w; x2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}
	x2 = w - 1;
	for (y2 = 0; y2 < h; y2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}
	t += micros() - start;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	x1 = w - 1;
	y1 = h - 1;
	y2 = 0;

	start = micros_start();

	for (x2 = 0; x2 < w; x2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}

	x2 = 0;
	for (y2 = 0; y2 < h; y2 += 6)
	{
		_cvbs.drawLine(x1, y1, x2, y2, color);
	}

	t += micros() - start;

	return t;
}

uint32_t testFastLines(uint8_t color1, uint8_t color2)
{
	uint32_t start;
	int32_t x, y;
	int32_t w = _cvbs.width();
	int32_t h = _cvbs.height();

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	start = micros_start();

	for (y = 0; y < h; y += 5)
		_cvbs.drawFastHLine(0, y, w, color1);
	for (x = 0; x < w; x += 5)
		_cvbs.drawFastVLine(x, 0, h, color2);

	return micros() - start;
}

uint32_t testRects(uint8_t color)
{
	uint32_t start;
	int32_t n, i, i2;
	int32_t cx = _cvbs.width() / 2;
	int32_t cy = _cvbs.height() / 2;

	_cvbs.fillScreen(TFT_BLACK);
	n = std::min(_cvbs.width(), _cvbs.height());
	start = micros_start();
	for (i = 2; i < n; i += 6)
	{
		i2 = i / 2;
		_cvbs.drawRect(cx-i2, cy-i2, i, i, color);
	}

	return micros() - start;
}

uint32_t testFilledRects(uint8_t color1, uint8_t color2)
{
	uint32_t start, t = 0;
	int32_t n, i, i2;
	int32_t cx = _cvbs.width() / 2 - 1;
	int32_t cy = _cvbs.height() / 2 - 1;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));
	n = std::min(_cvbs.width(), _cvbs.height());
	for (i = n; i > 0; i -= 6)
	{
		i2 = i / 2;

		start = micros_start();

		_cvbs.fillRect(cx-i2, cy-i2, i, i, color1);

		t += micros() - start;

		// Outlines are not included in timing results
		_cvbs.drawRect(cx-i2, cy-i2, i, i, color2);
	}

	return t;
}

uint32_t testFilledCircles(uint8_t radius, uint8_t color)
{
	uint32_t start;
	int32_t x, y, w = _cvbs.width(), h = _cvbs.height(), r2 = radius * 2;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	start = micros_start();

	for (x = radius; x < w; x += r2)
	{
		for (y = radius; y < h; y += r2)
		{
			_cvbs.fillCircle(x, y, radius, color);
		}
	}

	return micros() - start;
}

uint32_t testCircles(uint8_t radius, uint8_t color)
{
	uint32_t start;
	int32_t x, y, r2 = radius * 2;
	int32_t w = _cvbs.width() + radius;
	int32_t h = _cvbs.height() + radius;

	// Screen is not cleared for this one -- this is
	// intentional and does not affect the reported time.
	start = micros_start();

	for (x = 0; x < w; x += r2)
	{
		for (y = 0; y < h; y += r2)
		{
			_cvbs.drawCircle(x, y, radius, color);
		}
	}

	return micros() - start;
}

uint32_t testTriangles()
{
	uint32_t start;
	int32_t n, i;
	int32_t cx = _cvbs.width()/ 2 - 1;
	int32_t cy = _cvbs.height() / 2 - 1;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));
	n = std::min(cx, cy);

	start = micros_start();

	for (i = 0; i < n; i += 5)
	{
		_cvbs.drawTriangle(
			cx		, cy - i, // peak
			cx - i, cy + i, // bottom left
			cx + i, cy + i, // bottom right
			_cvbs.color565(0, 0, i));
	}

	return micros() - start;
}

uint32_t testFilledTriangles()
{
	uint32_t start, t = 0;
	int32_t i;
	int32_t cx = _cvbs.width() / 2 - 1;
	int32_t cy = _cvbs.height() / 2 - 1;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	start = micros_start();

	for (i = std::min(cx,cy); i > 10; i -= 5) {
		start = micros_start();
		_cvbs.fillTriangle(cx, cy - i, cx - i, cy + i, cx + i, cy + i,
			_cvbs.color332(0, i, i));
		t += micros() - start;
		_cvbs.drawTriangle(cx, cy - i, cx - i, cy + i, cx + i, cy + i,
			_cvbs.color332(i, i, 0));
	}

	return t;
}

uint32_t testRoundRects()
 {
	uint32_t start;
	int32_t w, i, i2;
	int32_t cx = _cvbs.width() / 2 - 1;
	int32_t cy = _cvbs.height() / 2 - 1;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	w = std::min(_cvbs.width(), _cvbs.height());

	start = micros_start();

	for (i = 0; i < w; i += 6)
	{
		i2 = i / 2;
		_cvbs.drawRoundRect(cx-i2, cy-i2, i, i, i/8, _cvbs.color332(i, 0, 0));
	}

	return micros() - start;
}

uint32_t testFilledRoundRects()
{
	uint32_t start;
	int32_t i, i2;
	int32_t cx = _cvbs.width() / 2 - 1;
	int32_t cy = _cvbs.height() / 2 - 1;

	_cvbs.fillScreen(_cvbs.color16to8(TFT_BLACK));

	start = micros_start();

	for (i = std::min(_cvbs.width(), _cvbs.height()); i > 20; i -= 6)
	{
		i2 = i / 2;
		_cvbs.fillRoundRect(cx-i2, cy-i2, i, i, i/8, _cvbs.color332(0, i, 0));
	}

	return micros() - start;
}

/***************************************************
  Original sketch text:

  This is an example sketch for the Adafruit 2.2" SPI display.
  This library works with the Adafruit 2.2" TFT Breakout w/SD card
  ----> http://www.adafruit.com/products/1480

  Check out the links above for our tutorials and wiring diagrams
  These displays use SPI to communicate, 4 or 5 pins are required to
  interface (RST is optional)
  Adafruit invests time and resources providing this open source code,
  please support Adafruit and open-source hardware by purchasing
  products from Adafruit!

  Written by Limor Fried/Ladyada for Adafruit Industries.
  MIT license, all text above must be included in any redistribution
 ****************************************************/

void begin() {
  Serial.begin(115200);
  Serial.println(""); Serial.println("");
  Serial.println("Lovyan's LovyanGFX library Test!");

  _cvbs.setRotation(0);
  _cvbs.init();
  _cvbs.fillScreen(0);
  _cvbs.waitForFrame();
}

void update(void)
{
	Serial.println(F("Benchmark                Time (microseconds)"));

	uint32_t usecHaD = testHaD();
	Serial.print(F("HaD pushColor            "));
	Serial.println(usecHaD);
	delay(100);

  uint32_t usecFillScreen = testFillScreen();
	Serial.print(F("Screen fill              "));
	Serial.println(usecFillScreen);
	delay(100);

  uint32_t usecText = testText();
	Serial.print(F("Text                     "));
	Serial.println(usecText);
	delay(100);

	uint32_t usecPixels = testPixels();
	Serial.print(F("Pixels                   "));
	Serial.println(usecPixels);
	delay(100);

	uint32_t usecLines = testLines(_cvbs.color16to8(TFT_BLUE));
	Serial.print(F("Lines                    "));
	Serial.println(usecLines);
	delay(100);

	uint32_t usecFastLines = testFastLines(_cvbs.color16to8(TFT_RED), _cvbs.color16to8(TFT_BLUE));
	Serial.print(F("Horiz/Vert Lines         "));
	Serial.println(usecFastLines);
	delay(100);

	uint32_t usecRects = testRects(_cvbs.color16to8(TFT_GREEN));
	Serial.print(F("Rectangles (outline)     "));
	Serial.println(usecRects);
	delay(100);

	uint32_t usecFilledRects = testFilledRects(_cvbs.color16to8(TFT_YELLOW), _cvbs.color16to8(TFT_MAGENTA));
	Serial.print(F("Rectangles (filled)      "));
	Serial.println(usecFilledRects);
	delay(100);

	uint32_t usecFilledCircles = testFilledCircles(10, _cvbs.color16to8(TFT_MAGENTA));
	Serial.print(F("Circles (filled)         "));
	Serial.println(usecFilledCircles);
	delay(100);

	uint32_t usecCircles = testCircles(10, _cvbs.color16to8(TFT_WHITE));
	Serial.print(F("Circles (outline)        "));
	Serial.println(usecCircles);
	delay(100);

	uint32_t usecTriangles = testTriangles();
	Serial.print(F("Triangles (outline)      "));
	Serial.println(usecTriangles);
	delay(100);

	uint32_t usecFilledTrangles = testFilledTriangles();
	Serial.print(F("Triangles (filled)       "));
	Serial.println(usecFilledTrangles);
	delay(100);

	uint32_t usecRoundRects = testRoundRects();
	Serial.print(F("Rounded rects (outline)  "));
	Serial.println(usecRoundRects);
	delay(100);

	uint32_t usedFilledRoundRects = testFilledRoundRects();
	Serial.print(F("Rounded rects (filled)   "));
	Serial.println(usedFilledRoundRects);
	delay(100);

	Serial.println(F("Done!"));

	uint16_t c = 4;
	int8_t d = 1;
	for (int32_t i = 0; i < _cvbs.height(); i++)
	{
		_cvbs.drawFastHLine(0, i, _cvbs.width(), c);
		c += d;
		if (c <= 4 || c >= 11)
			d = -d;
	}

	_cvbs.setCursor(0, 0);
	_cvbs.setTextColor(_cvbs.color16to8(TFT_MAGENTA));
	_cvbs.setTextSize(2);

 	_cvbs.println(F("  LovyanGFX test"));

	_cvbs.setTextSize(1);
	_cvbs.setTextColor(_cvbs.color16to8(TFT_WHITE));
	_cvbs.println(F(""));
	_cvbs.setTextSize(1);
	_cvbs.println(F(""));
	_cvbs.setTextColor(_cvbs.color332(0x80, 0x80, 0x80));

	_cvbs.println(F(""));


	_cvbs.setTextColor(_cvbs.color16to8(TFT_GREEN));
	_cvbs.println(F(" Benchmark               microseconds"));
	_cvbs.println(F(""));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW));

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("HaD pushColor      "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecHaD);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Screen fill        "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecFillScreen);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Text               "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecText);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Pixels             "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecPixels);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Lines              "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecLines);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Horiz/Vert Lines   "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecFastLines);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Rectangles         "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecRects);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Rectangles-filled  "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecFilledRects);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Circles            "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecCircles);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Circles-filled     "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecFilledCircles);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Triangles          "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecTriangles);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Triangles-filled   "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecFilledTrangles);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Rounded rects      "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usecRoundRects);

	_cvbs.setTextColor(_cvbs.color16to8(TFT_CYAN)); _cvbs.setTextSize(1);
	_cvbs.print(F("Rounded rects-fill "));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_YELLOW)); _cvbs.setTextSize(2);
	printnice(usedFilledRoundRects);

	_cvbs.setTextSize(1);
	_cvbs.println(F(""));
	_cvbs.setTextColor(_cvbs.color16to8(TFT_GREEN)); _cvbs.setTextSize(2);
	_cvbs.print(F("Benchmark Complete!"));

	delay(60 * 1000L);
}
